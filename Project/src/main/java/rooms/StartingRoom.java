...
public class StartingRoom extends TemplateRoom {
    ...
    public ArrayList<Tile> buildTiles() {
        // x + 30, y + 25
        ST_TILES = super.buildTiles();
        setOrigin(11, TileType.EXIT, ST_TILES);

        int[] unused = {1, 3, 6, 9, 10, 13, 17, 19, 20, 22, 23, 24, 25, 26, 27};
        removeUnused(unused, ST_TILES);
        int[] gold = {8};
        setTileType(gold, TileType.GOLD, ST_TILES);
        int[] monster = {4, 5, 12, 18};
        setTileType(monster, TileType.MONSTER, ST_TILES);
        int[] exits = {2, 16, 21};
        setTileType(exits, TileType.EXIT, ST_TILES);

        Tile e1 = ST_TILES.get(11);
        e1.setPathID(1);

        Tile e2 = ST_TILES.get(21);
        e2.setPathID(2);

        Tile e3 = ST_TILES.get(16);
        e3.setPathID(3);

        Tile e4 = ST_TILES.get(2);
        e4.setPathID(4);
